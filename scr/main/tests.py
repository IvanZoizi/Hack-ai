            ...
            for f in request.FILES.getlist('photos'):
                ...
                for x in range(0, w, SIDE):
                    ...
                    for y in range(0, h, SIDE):
                        ...
                        if y + SIDE > h:
                            y_dif = y + SIDE - h
                        print(img[y - y_dif:y + SIDE, x - x_dif:x + SIDE])
                        cv2.imwrite(f'img/SaveImage{x}_{y}.jpg', img[y - y_dif:y + SIDE, x - x_dif:x + SIDE])
            return render(request, 'image.html', {'form': form, 'image': f'img/SaveImage{x}_{y}.jpg'})
        else:
            return render(request, 'image.html', {'form': form})
